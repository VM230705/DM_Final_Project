...
def split_data(path, test_size=0.1, seed=42):
    ...
    tmp = data.loc[data['salary'].notna(), :]
    salary_retain = [(True if ('-' in txt and 'year' in txt) else False) for txt in tmp['salary']]
    result = tmp.loc[salary_retain, :].sample(frac=1, replace=False, axis=0, random_state=seed)
